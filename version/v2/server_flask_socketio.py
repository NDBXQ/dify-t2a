...
def receive_dify_msg(qurey, conversation_id="", userid="", secret=""):   
    """
    input: qurey 前端发送的消息
    output: dify处理之后的消息，会话id
    """

    response = dify_receive.get_response_message(qurey, conversation_id, userid, secret)
    msg, conversation_id, completion_tokens, prompt_tokens, total_tokens, message_id = dify_receive.handle_sse_response(response)
    return msg, conversation_id, completion_tokens, prompt_tokens, total_tokens, message_id
...
def handler(send, disconnect, message):
    ...
    if message.get("secret"):
        ...
        send('response',"未找得到secret键", room=online_users[userid])
    ...
    if message.get("conversation_id"):
        ...
        msg, conversation_id, completion_tokens, prompt_tokens, total_tokens, message_id = receive_dify_msg(qurey, conversation_id, userid, secret)
    else:
        msg, conversation_id, completion_tokens, prompt_tokens, total_tokens, message_id = receive_dify_msg(qurey, userid=userid, secret=secret)
...
if __name__ == '__main__':
    host = "0.0.0.0"
    port = 8765
    socketio.run(app, host=host, port=port, debug=False, allow_unsafe_werkzeug=True)  # 使用 socketio.run 替代 app.run
